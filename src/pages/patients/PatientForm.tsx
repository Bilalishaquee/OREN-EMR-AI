    ...
    const fetchData = async () => {
      ...
      try {
        if (isEditMode) {
          ...
          const dyn = patientData.dynamicData || {};
          const mappedData = {
            ...patientData,
            firstName: dyn["First Name"] || patientData.firstName || "",
            lastName: dyn["Last Name"] || patientData.lastName || "",
            dateOfBirth: dyn["Date of Birth"] || patientData.dateOfBirth || "",
            gender: dyn["Gender"] || patientData.gender || "",
            phone: dyn["Phone"] || patientData.phone || "",
            email: dyn["Email"] || patientData.email || "",
            assignedDoctor: dyn["assignedDoctor"] || patientData.assignedDoctor || "",
            address: {
              street: dyn["Address"] || patientData.address?.street || "",
              city: dyn["City"] || patientData.address?.city || "",
              state: dyn["State"] || patientData.address?.state || "",
              zipCode: dyn["ZIP Code"] || patientData.address?.zipCode || "",
              country: patientData.address?.country || "USA"
            },
            // You can add more mappings for other fields if needed
          };

          // Format date for input
          if (mappedData.dateOfBirth) {
            mappedData.dateOfBirth = new Date(mappedData.dateOfBirth).toISOString().split('T')[0];
          }

          // Ensure subjective structure
          if (!patientData.subjective) {
            mappedData.subjective = {
              fullName: '',
              date: '',
              physical: [],
              sleep: [],
              ...
              {user?.role === 'admin' && (
                <div>
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    className={`w-full px-3 py-2 border ${errors.assignedDoctor ? 'border-red-500' : 'border-gray-300'
                      } rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500`}
                  >
